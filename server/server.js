...
app.post("/login",upload.single('image'), (req, res) => {
  ...
  con.query(
    `select * from registaredUsers WHERE email = "${email.trim()}" && password = "${password.trim()}" `,
    (err, result, fields) => {
      if (err) throw err;
...
app.post("/registration",  (req, res) => {
  ...
  con.query(
    `select * from registaredUsers WHERE email = "${req.body.email}"`,
    (err, result, fields) => {
      if (result.length == 0) {
        const potourl =
          "https://media.istockphoto.com/vectors/default-profile-picture-avatar-photo-placeholder-vector-illustration-vector-id1223671392?k=20&m=1223671392&s=612x612&w=0&h=lGpj2vWAI3WUT1JeJWm1PRoHT3V15_1pdcTn2szdwQ0=";
        con.query(
          `INSERT INTO registaredusers (firstname,lastname,nickname,email,password,potorul) 
      VALUES (
        '${name}',
        '${lastname}',
        '${nickname}',
...
app.post("/byId", (req, res) => {
  con.query(
    `SELECT * FROM registaredusers WHERE ID = '${req.body.id}'`,
    (err, result) => {
      if (err) res.send("err");
      res.send(result);
    }
...
app.post("/updateImage", (req, res) => {
  con.query(
    `UPDATE registaredusers SET potorul= "${req.body.url}" WHERE ID = "${req.body.id}" `,
    (err, result) => {
      if (err) throw err;
      res.send("egaris");
    }
  ...
  con.query(
    ...
    (err, result) => {
      ...
      for (var i = 0; i < result.length; i++) {
        ...
        con.query(
          `delete from ${em}friends WHERE email = "${email}"`,
          (err, result) => {
            if (err) throw err;
          }
        );
      ...
      con.query(`drop table ${tableEmail}friends`, (err, result) => {
        con.query(
          `DELETE FROM registaredusers WHERE ID =${req.body.id} `,
          (err, result) => {
...
app.post("/addfriend", (req, res) => {
  con.query(
    `insert into ${req.body.emailForTable1} (email,name,potourl,lastname,nickname)
      VALUES(
      '${req.body.info2.email}',
      '${req.body.info2.firstname}',
      '${req.body.info2.potorul}',
      '${req.body.info2.lastname}',
      '${req.body.info2.nickname}')`,
    (err, result) => {
      if (err) console.log(err);
      con.query(
        `insert into ${req.body.emailForTable2} (email,name,potourl,lastname,nickname)
         VALUES(
          '${req.body.info1.email}',
          '${req.body.info1.firstname}',
          '${req.body.info1.potourl}',
          '${req.body.info1.lastname}',
          '${req.body.info1.nickname}')`,
        (err, result) => {
          if (err) console.log(err);
          res.send("inserted");
        }
...
app.post("/removeFriend", (req, res) => {
  con.query(
    `DELETE from ${req.body.emailForTable1} WHERE email = '${req.body.info2.email}'`,
    (err, result) => {
      if (err) console.log(err);

      con.query(
        `DELETE from ${req.body.emailForTable2} WHERE email = '${req.body.info1.email}'`,
        (err, result) => {
          if (err) console.log(err);
...
app.post("/getallfriendbyname", (req, res) => {
  con.query(
    `SELECT * from ${req.body.tableName}friends where name LIKE '%${req.body.name}%' && lastname LIKE "%${req.body.lastname}%"`,
    (err, result) => {
      if (err) {
        console.log(err);
      }
      if (result.length == 0) {
        con.query(
          `SELECT * from ${req.body.tableName}friends where name LIKE '%${req.body.name}%'`,
          (err, result) => {
            if (result.length == 0) {
              con.query(
                `SELECT * from ${req.body.tableName}friends where lastname LIKE '%${req.body.name}%'`,
                (err, result) => {
                  res.send(result);
                }
              );
...
app.post("/getallpeoplebyinfo", (req, res) => {
  ...
  con.query(
    `SELECT * from registaredusers where firstname LIKE '%${req.body.name}%' && lastname LIKE "%${req.body.lastname}%"`,
    (err, result) => {
      if (err) {
        console.log(err);
      }
      ...
      if (result.length == 0) {
        con.query(
          `SELECT * from registaredusers where firstname LIKE '%${req.body.name}%'`,
          (err, result) => {
            console.log("alo");
            console.log(result);
            if (result.length == 0) {
              console.log(result);
              con.query(
                `SELECT * from registaredusers where lastname LIKE '%${req.body.name}%'`,
                (err, result) => {
                  res.send(result);
                }
              );
  ...
  con.query(`SHOW TABLES LIKE '${tableName1}'`, (err, result) => {
    if (result.length == 0) {
      con.query(`SHOW TABLES LIKE '${tableName2}'`, (err, result) => {
        if (result.length == 0) {
          con.query(
            `CREATE TABLE ${tableName1} (mysms VARCHAR(255), recivedsms VARCHAR(255))`,
            (err, result) => {
              if (err) throw err;
              con.query(
                `CREATE TABLE ${tableName2} (mysms VARCHAR(255), recivedsms VARCHAR(255))`,
                (err, result) => {
                  if (err) throw err;
...
app.post("/sendsms", (req, res) => {
  ...
  con.query(
    `insert into ${table1Name} (mysms,recivedsms) VALUES('${sms}','${null}')`,
    (err, result) => {
      if (err) console.log(err);
      console.log(result);
      con.query(
        `insert into ${table2Name} (mysms,recivedsms) VALUES('${null}','${sms}')`,
        (err, result) => {
          if (err) console.log(err);
          console.log(result);
          res.send("sent");
