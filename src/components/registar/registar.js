...
const Registar = ()=>{
  ...
  const submitHandler = (event)=>{
    ...
    if(classEmail != 'invalid' && classpassword != 'invalid' && classNickName != 'invalid'  && classname != 'invalid' && classLastname != 'ivalid'){
      registration(nameInput.trim(), lastnameInput.trim(), nickNameInput.trim(), emailInput.trim(), passwordInput.trim())
    }
    else{
      setError(true)
  ...
  const {
      ...
      focusHandler:lostFocuspassword} = InputHook('password')
  ...
  const {
        ...
        focusHandler:lostFocusNickname} = InputHook('nickname') 

  const {
          inputValue : lastnameInput,
          nameClassName : classLastname,
          changeHandlerFun : lastnameChangeHandler,
          focusHandler:lostFocuslastname} = InputHook('lastname')        
        ...
        <div class="input-field">
          <input className={classname}
           onChange= {nameChangeHandler}
            onBlur = {lostFocusname} type="text" placeholder="Name" autocomplete="nope" />
        </div>
        <div class="input-field">
          <input className={classLastname}
           onChange ={lastnameChangeHandler}
            onBlur = {lostFocuslastname} type="text" placeholder="Lastname" autocomplete="nope" />
        </div>
        <div class="input-field">
          <input className={classNickName}
           onChange ={nicknamechangeHandler}
            onBlur = {lostFocusNickname} type="text" placeholder="Nickname" autocomplete="nope" />
        </div>
        <div class="input-field">
          <input className={classEmail}
           onChange ={EmailChangeHandler} 
           onBlur = {lostFocusEmail} type="email" placeholder="Email" autocomplete="nope" />
        </div>
        <div class="input-field">
          <input className={classpassword}
           onChange ={passwordChangeHandler}
            onBlur = {lostFocuspassword} type="password" placeholder="Password" autocomplete="new-password" />
        </div>
      </div>
      <div class="action">
