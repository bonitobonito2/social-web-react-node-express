...
const InputHook = (type)=>{
    ...
    const changeHandler = (event)=>{
        ...
        if(type ==='name') isValid = inputValue.trim() !== '' && inputValue.length > 2
        if(type ==='lastname') isValid = inputValue.trim() !== '' && inputValue.length > 5
        
        if(type === 'email' ){
            isValid = inputValue.trim() !== '' && inputValue.includes("@") &&  inputValue.includes(".")  && inputValue.length > 6
        } 
        
        if(type === 'nickname') isValid = inputValue.trim() !== '' && inputValue.length > 5
