...
const Change = (props) => {
  ...
  const submitHandler = (event) => {
    ...
    if (information["password"] == password.current.value.trim()) {
      if (
        firstname.current.value.trim().length > 3 &&
        lastname.current.value.trim().length > 5 &&
        nickname.current.value.trim().length > 5
      ) {
        change(
    ...
    <Fragment>
      ...
      <div className={classes.card}>
        <form onSubmit={submitHandler}>
          ...
          <br />
          <input
            onChange={() => setFirstname1(firstname.current.value)}
            ref={firstname}
            type="text"
            value={firstname1}
          />
          ...
          <input
            onChange={() => setLastname1(lastname.current.value)}
            ref={lastname}
            type="text"
            value={lastname1}
          />
          ...
          <input
            onChange={() => setNickname1(nickname.current.value)}
            ref={nickname}
            type="text"
            value={nickname1}
          />
