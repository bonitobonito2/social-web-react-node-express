...
const Friends = (props)=>{
    ...
    useEffect(()=>{
        ...
        axios.post('http://localhost:5000/getallfriend',{tableName : myEmailForTable })
        .then(response=>{
    ...
    const changeHandler =()=>{
        ...
        axios.post('http://localhost:5000/getallfriendbyname',{tableName : myEmailForTable, name : name, lastname : lastname })
        .then(response=>{
            if(response.data.length == 0){
                setFriends([])
            }else{
    ...
    return <div className={classes.card}>
        ...
        <div className={classes.inputDiv}>
            <center>
            <input onChange={changeHandler} ref={friendName} placeholder='search friend'/>
            </center>
